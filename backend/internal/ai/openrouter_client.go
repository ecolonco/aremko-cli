...
// actionsClosingInstruction obliga a que cada análisis ejecutivo cierre en una
// lista FINITA (7-10) de acciones concretas y priorizadas (Etapa 1 de informes).
const actionsClosingInstruction = `
...
// wrapAnalysisPrompt es como wrapSystemPrompt pero además obliga a cerrar el
// análisis con la lista finita de acciones priorizadas. Se usa solo en los
// análisis ejecutivos (no en NL query / brief legacy / calendario de contenido).
func (c *OpenRouterClient) wrapAnalysisPrompt(base string) string {
	return c.wrapSystemPrompt(base) + actionsClosingInstruction
}
...
⚠️ REGLA CRÍTICA — VENTAS REALES vs. actividad de la plataforma (aplica a ritual, refugio y pausa, y a TODO el informe incluido el Veredicto): estos bloques pueden traer un arreglo "weekly" con las últimas 8 semanas (label, spend, activity [conversaciones o leads que ve ESTA plataforma], reservas, ingresos). reservas/ingresos son VENTAS REALES confirmadas en el sistema de reservas de Aremko, de TODOS los canales de venta (no solo lo que Meta atribuye a este anuncio) — son la fuente de verdad de si el programa se está vendiendo. spend/activity son solo lo que Meta ve y puede subregistrar. Si activity=0 o es bajo pero weekly muestra reservas/ingresos reales, JAMÁS concluyas "sin retorno" ni recomiendes pausar el producto — la lectura correcta es una brecha de ATRIBUCIÓN/medición (el clic o mensaje no se está registrando como conversión), no un fracaso de la oferta. Calcula y reporta el retorno DIRECCIONAL de cada programa: suma de ingresos de las 8 semanas ÷ suma de spend de las 8 semanas de ESA plataforma, aclarando siempre que es direccional (el ingreso agrega todos los canales de venta, no es 100% atribuible a este anuncio) y sin confundirlo con un ROAS de plataforma. Si el negocio vende bien (weekly sano) pero la plataforma no lo mide, el Veredicto correcto es 🟡 (brecha de medición), NUNCA 🔴 (fracaso) — reserva 🔴 para cuando ni las ventas reales ni la plataforma muestran resultado.`
...
### Ventas reales del programa (weekly) — SOLO SI meta_ads.refugio.weekly existe
Aplica la misma REGLA CRÍTICA del rol: reporta reservas e ingresos reales de las 8 semanas (todos los canales de venta) y el retorno direccional (ingresos ÷ spend de Meta). Si leads=0 o el CPL es altísimo pero weekly muestra reservas reales, la conclusión es brecha de atribución (el lead-formulario no captura toda la venta real), NO que Refugio no vende.
...
### Ventas reales del programa (weekly) — reportar SIEMPRE que exista
Si el bloque trae weekly (8 semanas), antes de la recomendación reporta: (1) total de reservas e ingresos reales de las 8 semanas; (2) la semana más reciente vs el promedio de las 7 anteriores — ¿el programa acelera o desacelera en ventas, más allá del gasto en Meta?; (3) el retorno direccional 8 semanas (ingresos ÷ spend de Meta), aclarando que agrega todos los canales de venta; (4) si activity (conversaciones) viene en 0 o bajo pero reservas>0, decláralo EXPLÍCITAMENTE como brecha de atribución (ver REGLA CRÍTICA del rol), nunca como fracaso de la campaña o del producto.

### Recomendación accionable (UNA por campaña)
- Una sola decisión por campaña, anclada a números. Si es muy temprano (días<4), la recomendación válida es "mantener y observar, aún en aprendizaje". Recuerda el contexto de precio (Ritual premium $210K / Pausa entrada $110K) al juzgar si el costo por conversación es sano. Si hay brecha de atribución (weekly con ventas reales pero activity=0), la recomendación prioritaria es arreglar la medición, no pausar ni bajar presupuesto.
...
Genera un análisis EJECUTIVO PROFUNDO siguiendo EXACTAMENTE la estructura del system prompt. Sin restricción de extensión: si los datos justifican 4-6 páginas, escríbelo. Prioriza cruzar múltiples datasets en cada sección sobre brevedad. Cada afirmación debe anclarse a números concretos del payload.

Recordatorio antes de escribir: revisa si ritual/refugio/pausa traen weekly con reservas/ingresos reales ANTES de calificar el retorno como "cero" o recomendar pausar — activity=0 (conversaciones/leads) de Meta puede ser una brecha de medición, no evidencia de que el programa no vende. Ver REGLA CRÍTICA del system prompt.`, time.Now().Format("2006-01-02"), string(dataJSON))
...
⚠️ REGLA CRÍTICA — LÉELA ANTES DE ESCRIBIR EL VEREDICTO (aplica a TODO el informe, no solo a una sección): el lead real de Aremko llega por WhatsApp, y HOY ese evento NO está importado como conversión en Google Ads (brecha de medición conocida, ya reportada al equipo — no es un hallazgo nuevo que debas "descubrir" cada vez). Esto significa que conversions=0 o CPL indefinido en el payload de Google Ads NO implica que no hubo ventas — solo que Google no las ve. Cada bloque ritual/refugio/pausa puede traer un arreglo weekly (últimas 8 semanas: spend, activity [clics de Google], reservas, ingresos), donde reservas/ingresos SON LAS VENTAS REALES confirmadas en el sistema de reservas de Aremko, de TODOS los canales de venta (no solo Google) — esa es la fuente de verdad de si el programa vende. PROHIBIDO escribir "sin retorno directo", "cero conversiones = fracaso" o recomendar "pausar la inversión" basándote solo en conversions=0 de Google cuando weekly.reservas muestra ventas reales — en ese caso el diagnóstico correcto es "brecha de atribución/tracking" (acción prioritaria: arreglar el tracking, no pausar gasto) y el Veredicto debe ser 🟡 (brecha de medición) o incluso 🟢 si las ventas reales son sanas, NUNCA 🔴 (fracaso de negocio) solo por el conversions=0 de la plataforma. Reserva 🔴 para cuando NI las ventas reales (weekly) NI la plataforma muestran resultado.`

	domainStructure := `## 🌿 Refugio Search — SOLO SI EL PAYLOAD TRAE google_ads.refugio
Si existe google_ads.refugio, va primero: gasto vs presupuesto (summary.budget_pct_used), conversiones y CPL (summary.cpl) vs umbral verde <$10.000 CLP. Términos de búsqueda (search_terms): 3 con mejor CPL + negativas sugeridas (búsquedas con clics y 0 conversiones). Quality Score (quality_scores): keywords con QS<5 encarecen el CPC 25-50% → recomendar mejorar relevancia de anuncio/landing. ANTES de calificar el CPL/conversiones como "sin retorno", aplica la REGLA CRÍTICA del rol: si google_ads.refugio.weekly existe y muestra reservas reales, la conclusión es brecha de atribución, no fracaso.

## 🔍 Campañas de Search (todas)
Para cada campaña en google_ads.campaigns: gasto, clics, conversiones, CPL. Identifica por NOMBRE las nuevas — "Ritual del Río - Search" y "Pausa junto al río - Search" — y su arranque. REGLA DE VOLUMEN: si una campaña tiene <5 clics en la semana, dilo SIN alarmar (mercado chico + keywords en "bajo volumen"); la palanca es priorizar keywords amplias + subir puja/presupuesto, NO declarar fracaso. Si una campaña tiene 0 datos porque recién se lanzó, márcala como "en activación". ANTES de escribir "sin conversiones", "0 conversiones" o cualquier frase que suene a fracaso para Ritual/Refugio/Pausa, revisa si el bloque correspondiente (google_ads.ritual/.refugio/.pausa) trae weekly con reservas>0 — si las hay, la campaña no está "fallando", está en brecha de atribución (ver REGLA CRÍTICA del rol); refleja eso en el texto en vez de alarmar.

## 🌙🌿🍃 Ventas reales vs. conversiones de Google (weekly) — sección obligatoria si algún bloque trae weekly
Para CADA bloque presente (google_ads.ritual, google_ads.refugio, google_ads.pausa) que traiga weekly (8 semanas): reporta (1) total de reservas e ingresos reales de las 8 semanas y de la semana más reciente; (2) el retorno direccional = suma ingresos 8 semanas ÷ suma gasto Google 8 semanas, aclarando SIEMPRE que agrega ventas de TODOS los canales (no es ROAS puro de Google); (3) si conversions/CPL de Google es 0/indefinido pero reservas>0, decláralo EXPLÍCITAMENTE como brecha de atribución (el evento de WhatsApp no está importado a Google Ads), nunca como fracaso del programa. Esta sección es la que reconcilia el Veredicto con la realidad de ventas — si hay contradicción entre "conversions=0" y "reservas>0", gana la lectura de reservas reales para el tono general del informe.
...
Genera un análisis EJECUTIVO siguiendo EXACTAMENTE la estructura del system prompt. Ancla cada afirmación a números del payload. Si el volumen es bajo (mercado chico / keywords en bajo volumen), dilo sin alarmar y enfócate en cómo destrabar volumen (keywords amplias, presupuesto, Quality Score), no en pausar.

Recordatorio antes de escribir: revisa si ritual/refugio/pausa traen weekly con reservas/ingresos reales ANTES de calificar el retorno como "cero" o recomendar pausar — conversions=0 en Google es una brecha de medición conocida (el lead de WhatsApp no está importado), no evidencia de que el programa no vende. Ver REGLA CRÍTICA del system prompt.`, time.Now().Format("2006-01-02"), string(dataJSON))
